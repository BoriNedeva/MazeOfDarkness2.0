...
public class Maze {
	...
	private int cols; // cols in the repres. feild

	private int act_cols; // actual cols in the maze
	private int act_rows; // actual rows in the maze

	private int entrance_row = 0;
	private int exit_row = 0;

	private ArrayList<Coordinates> emptyFieldCoords = new ArrayList<Coordinates>();
	// private ArrayList<Coordinates> wallCoords = new ArrayList<Coordinates>();
	...
	public ArrayList<Coordinates> getEmptyFieldCoords() {
	...
	private void fillCoordinates() {
		for (int i = 0; i < field.length; i++) {
			for (int j = 0; j < field[i].length; j++) {
				if (field[i][j] == WALL) {
					// wallCoords.add(new Coordinates(i,j));
				} else if (field[i][j] == PATH) {
					emptyFieldCoords.add(new Coordinates(i, j));
				}
			}
		}
	}
	...
	private void setInitialLocation() {
		...
		field[startRowIndex][startColIndex] = PATH;
	}
	...
	private int canUp(Integer[] current) {
		...
		if (nxt_row < 0 || field[nxt_row][nxt_col] == PATH) {
			return 0;
		} else {
	...
	private int canDown(Integer[] current) {
		...
		if (nxt_row > rows - 1 || field[nxt_row][nxt_col] == PATH) {
			return 0;
		} else {
	...
	private int canRight(Integer[] current) {
		...
		// if the next location is in the array and not already cut, can cut it
		if (nxt_col > cols - 1 || field[nxt_row][nxt_col] == PATH) {
			return 0;
		} else {
	...
	private int canLeft(Integer[] current) {
		...
		// if next location is in the array and not already cut, can cut it
		if (nxt_col < 0 || field[nxt_row][nxt_col] == PATH) {
			return 0;
		} else {
	...
	private int cutNextUp(Integer[] current) {
		...
		field[current[0] - 1][current[1]] = PATH;
		field[current[0] - 2][current[1]] = PATH;
	...
	private int cutNextRight(Integer[] current) {
		...
		field[current[0]][current[1] + 1] = PATH;
		field[current[0]][current[1] + 2] = PATH;
	...
	private void makeOpenings() {
		...
		this.entrance_row = rand.nextInt(act_rows - 1) * 2 + 1;
		this.exit_row = rand.nextInt(act_rows - 1) * 2 + 1;
		// clear the location
	...
	public static void main(String[] args) {
		...
		maze.printMaze();
			...
			System.out.println(maze.getEmptyFieldCoords().size());
